...
public class TmdbTvShowSearchService implements TvShowSearchService {
    ...
    public SearchResult<MediaFullDetails> search(SearchParams searchParams) throws ExternalAPIException {
        ...
        for (BaseTvShow result : movieResults.results) {
            try {
                ...
                LOG.warn("Failed to load details for tv show [name: {}, id: {}]. Ignoring in the search result.", result.name, result.id, e);
            }
        }
    ...
    private TvShowResultsPage searchMedia(SearchParams searchParams, Tmdb tmdb) throws ExternalAPIException {
        try {
